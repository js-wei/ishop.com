...
class Lists extends TagLib{
    protected $tags = array(
         ...
         'channels'=>array('attr' => 'model,fid,field,limit,where,order,position','close' =>1),
    ...
    public function _list ($attr,$content){
      ...
      $order=$attr['order'];//$order$limit，可通过模板传入参数值
      $where= $this->adjunct($attr);
      $id_=!empty($attr['id'])?$attr['id']:I('id');
      $attr=!empty($attr['attr'])?$attr['attr']:'';

      $id = $this->_get_child($id_);
    ...
    public function _list2 ($attr,$content){
      ...
      $id=!empty($attr['id'])?$attr['id']:I('id');
	    $session_column_id = session('session_column_id');
		
      if(!strstr($where,'column_id')){
		  if(empty($_REQUEST['id'])){  
			  if(!empty($id)){
				if(!empty($session_column_id)){
					$this->_get_child();
					$where.=' and column_id = '.$session_column_id;
				}else{
					if(!empty($where)){
					  $where.=' and column_id = '.$id;
					}else{
				...
				$cid = $this->_get_parent($session_column_id);
				$where.=$cid?' and column_id in ('.$cid.')':'';	
			  }
		  }else{ 
			  if(!empty($session_column_id)){ 
					 $id = $this->_get_child($session_column_id);
					 $where.=$id?' and column_id in ('.$id.')':'';	
				  }else{
    ...
    public function _article($attr,$content){
      ...
      $id=!empty($attr['id'])?$attr['id']:I('aid');
      $where= $this->condition($attr['where']);
  ...
  public function _channellist($attr,$content){
      ...
      $str='<?php 
      	  $parent=M("'.$model.'")->field("'.$field.'")->find('.$fid.');
         $_result_channellist=M("'.$model.'")->field("'.$field.'")->where("'.$where.'")->order("'.$order.'")->limit("'.$limit.'")->select();
         $channellist=\Service\Category::unlimitedForLevel($_result_channellist);
      ?>';
      $str .= $content;
      return $str;
  }
  ...
  public function _channels($attr,$content){
      ...
      $str='<?php 
      	  $this_cc = M("'.$model.'")->field("'.$field.'")->find('.$fid.');
          $_result_channellist=M("'.$model.'")->field("'.$field.'")->where("'.$where.'")->order("'.$order.'")->limit("'.$limit.'")->select();
		  $this_cc["child"]=$_result_channellist;
		  $_result_channellist =$this_cc;
          $channels=$_result_channellist;
      ?>';
      $str .= $content;
      return $str;
  }
