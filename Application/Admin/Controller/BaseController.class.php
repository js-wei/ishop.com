...
class BaseController extends Controller {
	 ...
	 * @descritopn 获取缩略图地址
	 */
	protected function get_thumb_path($image){
		$_reuslt = '';
		$path = explode('/', $image);
		$count = count($path)-1;
		foreach($path as $k=>$v){
    ...
    protected function _breadcrumb(){
       $control=strtolower(CONTROLLER_NAME);
		if(I('get.id')==''){
			$map['title']=$control;
			$model = M('model')->field('id,title,name')->where($map)->find();
			$breadcrumb = M('model')->field('id,title,name')->where(array('title'=>$map))->find();
		}else{
			$map['id']=I('get.id');
			$model = M('column')->field('id,title,fid')->where($map)->find();
			$model['name']=$model['title'];
			$breadcrumb = M('column')->field('id,title')->where(array('id'=>$model['fid']))->find();
    ...
    */
    ...
    protected function _status($id,$model='',$t='',$img='image'){
        ...
        $ajax=I('ajax','',intval);
        $ajax=($ajax==1)?1:0;
        $r = I('get.cid')?I('get.cid'):I('get.id');
        switch($t){
            case 'enable':            //启用
	...
	/**
	 * @author 魏巍
	 * @descrition kindegitor自动保存
	 */
    public function auto_kindeditor_save($id=0,$content=''){
        \Vin\FileStorage::connect();
        $rootpath = '/Data/Storage/';
        $filename = $id.'.stg';
        if(I('post.get')!=1 && !empty($content)){
            \Vin\FileStorage::put($rootpath,$filename,$content);
            $this->ajaxReturn(['status'=>1,'msg'=>'(:已经在云端进行了保存']);
        }else{
            $content = \Vin\FileStorage::get($rootpath,$filename);
			if(!empty($content)){
				$this->ajaxReturn(['status'=>1,'msg'=>'<b>:)</b>您有一篇自动保存的文档在服务器端,请问您是否进行否恢复?','content'=>$content,'path'=>$rootpath.$filename]);
			}else{
				$this->ajaxReturn(['status'=>0,'msg'=>':)服务器端没有保存的文档']);
			}	
        }
    }
	/**
	 * @author 魏巍
	 * @descrition 删除kindegitor自动保存
	 */
	public function auto_kindeditor_clear($id){
		\Vin\FileStorage::connect();
        $rootpath = '/Data/Storage/';
        $filename = $id.'.stg';
		if(\Vin\FileStorage::del($rootpath,$filename)){
			$this->ajaxReturn(['status'=>1,'msg'=>'操作成功','content'=>$content,'path'=>$rootpath.$filename]);
		}
	}
    /**
     * @return array
     ...
     */
    protected function makeAttr($resetAttr,$t=0){
        $attr=$t?array('com'=>0,'new'=>0,'head'=>0,'top'=>0,'img'=>0,'hot'=>0):array('requird'=>0,'connect'=>0,'email'=>0,'url'=>0);
