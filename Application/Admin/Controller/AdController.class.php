...
class AdController extends BaseController{
	...
	public function index(){
        ...
        //获取数据
		$this->arclist=$arclist=$this->getlist(M('Ad'), $map, $ordermap);
		$this->display();
	}
	...
	public function add($id=0){
		...
		$this->column_list = $list = \Service\Category::LimitForLevel(M('column')->order('id asc')->where('status=0')->select());
		$this->display();
	}
	/**
	 * [status 修改状态]
	...
	public function add_ad(){
		...
		$data = $this->_param();
		$data['effective']=strtotime($data['effective']);
