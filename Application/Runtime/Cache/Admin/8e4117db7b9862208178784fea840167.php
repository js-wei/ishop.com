                ...
                <div class="span12">
                    <div class="portlet box purple">
                        <div class="portlet-title">
                            <div class="caption"><i class="icon-reorder"></i><?php echo ((isset($model["name"]) && ($model["name"] !== ""))?($model["name"]):'基本配置'); ?></div>
                            <div class="tools">
                        ...
                        <div class="portlet-body form">
                            <!-- BEGIN FORM-->
                            <form action="/Admin/Config/addhandler" method="post" id="form_sample_3" class="form-horizontal" novalidate="novalidate" enctype="multipart/form-data">
                                <div class="alert alert-error hide">
                                    <button class="close" data-dismiss="alert"></button>
                                    请将必填的填写完整
                                </div>
                                <div class="control-group">
                                    <label class="control-label">网站标题(title)<span class="required">*</span></label>
                                    <div class="controls">
                                        <input type="text" name="title" id="control-name" data-required="1" value="<?php echo ($conf["title"]); ?>" class="span4 m-wrap" placeholder="网站标题">
                                    </div>
                                </div>
								<div class="control-group">
									<label class="control-label">网站LOGO(logo)</label>
									<div class="controls">
										<div>
										...
										<div>
											<?php if(!empty($conf["logo"])): ?><img src="<?php echo ($conf["logo"]); ?>" id="images_preview" width="220" height="auto">
												<input type="hidden" value="<?php echo ($conf["logo"]); ?>" name="logo" id="reply_img">
												<?php else: ?>
												<img src="" id="images_preview" width="220" height="auto" style="display:none;">
												<input type="hidden" name="logo" id="reply_img"><?php endif; ?>
										</div>
									</div>
								</div>
                                <div class="control-group">
                                    <label class="control-label">网站地址(url)</label>
                                    <div class="controls">
                                        <input type="text" name="url" value="<?php echo ($conf["url"]); ?>" placeholder="网站地址"/>
                                        <input type="hidden" name="id" value="<?php echo ($conf["id"]); ?>"/>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">网站关键词(keywords)</label>
                                    <div class="controls">
                                        <textarea name="keyword" class="span8 m-wrap" rows="5"  placeholder="网站关键词"><?php echo (htmlspecialchars_decode($conf["keywords"])); ?></textarea>
										<div class="text-error">*非常重要最好填写,字数最好在120字内,详细请百度"搜索引擎关键词"词条</div>
									</div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">网站介绍(description)</label>
                                    <div class="controls">
                                        <textarea name="description" class="span8 m-wrap" rows="5" placeholder="网站介绍"><?php echo (htmlspecialchars_decode($conf["description"])); ?></textarea>
										<div class="text-error">*很重要最好填写,字数最好在250字内,详细请百度"搜索引擎描述"词条</div>
									</div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">开启轮播(carousel)</label>
                                    <div class="controls">
                                        <label class="radio">
                                            <input type="radio" name="carousel"  value="0" <?php if(($conf["carousel"]) == "0"): ?>checked<?php endif; ?> >开启
                                        </label>
                                        <label class="radio">
                                            <input type="radio" name="carousel"   value="1" <?php if(($conf["carousel"]) == "1"): ?>checked<?php endif; ?>>禁用
                                        </label>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">首页幻灯数(sum)</label>
                                    <div class="controls">
                                        <input type="text" value="<?php echo ($conf["sum"]); ?>" name="sum" placeholder="首页幻灯数">
                                        <div class="text-error">不可为负数</div>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">友情链接(flink)</label>
                                    <div class="controls">
                                        <input type="text" value="<?php echo ($conf["flink"]); ?>" name="flink" placeholder="友情链接">
                                        <div class="text-error">0表示不调用;否则调用个数,不可为负数</div>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">版权信息(copyright)</label>
                                    <div class="controls">
                                        <input type="text" name="copyright" value="<?php echo ($conf["copyright"]); ?>" placeholder="版权信息"/>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">备案号(icp)</label>
                                    <div class="controls">
                                        <input type="text" name="icp" value="<?php echo ($conf["icp"]); ?>" placeholder="备案号"/>
                                    </div>
                                </div>
								<div class="control-group">
                                    <label class="control-label">电话(tel)</label>
                                    <div class="controls">
                                        <input type="text" name="tel" value="<?php echo ($conf["tel"]); ?>" placeholder="电话/传真"/>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">传真(fax)</label>
                                    <div class="controls">
                                        <input type="text" name="fax" value="<?php echo ($conf["fax"]); ?>" placeholder="传真"/>
                                    </div>
                                </div>
								<div class="control-group">
                                    <label class="control-label">联系手机(phone)</label>
                                    <div class="controls">
                                        <input type="text" name="phone" value="<?php echo ($conf["phone"]); ?>" placeholder="联系手机"/>
                                    </div>
                                </div>
								<div class="control-group">
                                    <label class="control-label">联系地址(address)</label>
                                    <div class="controls">
                                        <input type="text" name="address" value="<?php echo ($conf["address"]); ?>" placeholder="联系地址"/>
                                    </div>
                                </div>
								<div class="control-group">
                                    <label class="control-label">联系邮箱(email)</label>
                                    <div class="controls">
                                        <input type="text" name="email" value="<?php echo ($conf["email"]); ?>" placeholder="联系邮箱"/>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">其他配置扩展(extends)</label>
                                    <div class="controls">
                                        <textarea name="extends" class="span8 m-wrap" rows="8" placeholder="其他配置扩展"><?php echo (htmlspecialchars_decode($conf["extends"])); ?></textarea>
										<div class="text-error">*用于对网站配置的扩展,配置之间使用英文','隔开,属性与值之间使用英文':'隔开.如"qq:524314430,gmail:jswei30@gmail.com".这样您就扩展了名为:'qq'和'gmail'的参数.就看使用它们了.</div>
									</div>
                                </div>
								<div class="control-group">
                                    <label class="control-label">开启分享(is_shard)</label>
                                    <div class="controls">
                                        <label class="radio">
                                            <input type="radio" name="is_shard"  value="0" <?php if(($conf["is_shard"]) == "0"): ?>checked<?php endif; ?> >开启
                                        </label>
                                        <label class="radio">
                                            <input type="radio" name="is_shard"   value="1" <?php if(($conf["is_shard"]) == "1"): ?>checked<?php endif; ?>>禁用
                                        </label>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">分享代码(shard)</label>
                                    <div class="controls">
                                        <textarea name="shard" class="span8 m-wrap" rows="8" placeholder="分享代码"><?php echo (htmlspecialchars_decode($conf["shard"])); ?></textarea>
                                    </div>
                                </div>
                                <div class="control-group">
                                    <label class="control-label">统计代码(code)</label>
                                    <div class="controls">
                                        <textarea name="code" class="span8 m-wrap" rows="8" placeholder="统计代码"><?php echo (htmlspecialchars_decode($conf["code"])); ?></textarea>
                                    </div>
                                </div>
	...
	<script type="text/javascript">
        ...
        $('#btn-enable,#btn-forbidden,#btn-delete').click(function(e){
            ...
            if($q!=''){
                ...
                $.post('/Admin/Config/status',{k:$q,t:$t,p:$p},function(data){
                    window.location.reload();
                });
            }else{
